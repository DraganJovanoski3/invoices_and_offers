-- Add legal fields to invoices table (supply_date, authorized_person, payment_method, fiscal_receipt_number)
...
SET @sqlstmt := IF(@exist = 0, 
    'ALTER TABLE invoices ADD COLUMN authorized_person VARCHAR(100) DEFAULT NULL',
    'SELECT 1');
PREPARE stmt FROM @sqlstmt;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sqlstmt := IF(@exist = 0, 
    'ALTER TABLE invoices ADD COLUMN fiscal_receipt_number VARCHAR(100) DEFAULT NULL',
    'SELECT 1');
PREPARE stmt FROM @sqlstmt;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
